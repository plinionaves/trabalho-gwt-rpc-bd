...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "modulo" because\n';
  ...
  $wnd.__gwt_activeModules['modulo'] = {
    'moduleName' : 'modulo',
    'bindings' : function() {
      return {};
    }
 ...
 * "/modulo.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['modulo'].superdevmode = true;

  var expectedSuffix = '/modulo.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + modulo + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:modulo';
  var devModeSessionKey = '__gwtDevModeSession:modulo';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/modulo/modulo.nocache.js';
  ...
  $wnd.__gwt_activeModules['modulo'].canRedirect = true;
  $wnd.__gwt_activeModules['modulo'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/modulo?user.agent=' + ua + '&_callback=' + callback;
